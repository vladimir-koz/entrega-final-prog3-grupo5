...
function Rutinas() {
  ...
  async function expand(routine) {
    if (expandedId === routine.id) {
      setExpandedId(null);
      return;
    }
    setExpandedId(routine.id);
    if (!details[routine.id]) {
      try {
        const data = await getRoutineExercises(routine.id);
        setDetails((current) => ({ ...current, [routine.id]: data }));
      } catch (requestError) {
  ...
  async function openEdit(routine) {
    const routineLines = details[routine.id] || (await getRoutineExercises(routine.id));
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (editingId) {
        for (const existing of details[editingId] || []) await deleteRoutineExercise(existing.id);
      }
      const savedLines = [];
