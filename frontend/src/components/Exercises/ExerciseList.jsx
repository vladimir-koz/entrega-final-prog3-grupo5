            ...
            {exercises.map((exercise) => {
                ...
                return (
                    <article className="resource-item" key={exercise.id}>
                        <button className="resource-summary" onClick={() => onSelect(expanded ? null : exercise.id)}>
                            <span><strong>{exercise.nombre}</strong><small>{owned ? "Personal" : "PowerUp"} · {exercise.dificultad || "Sin dificultad"}</small></span>
                        ...
                        {expanded && (
                            <div className="resource-detail">
                                <div>
                                    ...
                                    <div className="tag-row">{exercise.muscleGroups?.map((group) => <span key={group.id}>{group.nombre}</span>)}
                                    </div>
                                </div>
                                {owned && <div className="inline-actions">
                                    <button
