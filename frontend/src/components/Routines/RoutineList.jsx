          ...
          <article className="resource-item" key={routine.id}>
            <button className="resource-summary" onClick={() => onExpand(routine)}>
              <span>
                ...
                <small>
                  {owned ? "Personal" : "PowerUp"} ·{" "}
                  {routine.tiempoEstimado ? `${routine.tiempoEstimado} min` : "Duración libre"}
                </small>
              </span>
            ...
            {expanded && (
              <div className="resource-detail">
                <div>
                  ...
                  <ol className="routine-lines">
                    {(details[routine.id] || []).map((line) => (
                      <li key={line.id}>
                        <span>{line.exercise?.nombre}</span>
                        <strong>
                          {line.repeticiones} reps · {line.peso ?? 0} kg
                        </strong>
                      </li>
                    ))}
                  </ol>
                ...
                {owned && (
                  <div className="inline-actions">
                    <button className="secondary-button" onClick={() => onEdit(routine)}>
